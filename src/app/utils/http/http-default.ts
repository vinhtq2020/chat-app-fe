...
import { UserAgentContext } from "../../components/providers"
...
export class HttpService {
    ...
    private async sendRequest(url: string, options: RequestInit) {
        ...
        if (this.isRefreshing) {
            ...
            const res = await fetch(url, mergeOptions).finally(() => clearTimeout(this.requestTimeout))
            return this.handleResponse(res, url, options)
        }

    }

    private async handleResponse(response: Response, url: string, options: RequestInit) {
        if (this.interceptors.response.onIntercepterResponse) {
            return this.interceptors.response.onIntercepterResponse(response, url, options)
        }
        return response
    }
...
httpInstance.interceptors.response.use(
    async (response, url, options) => {
        if (response.status == 401) {
            ...
            const userAgent = use(UserAgentContext)
            getAuthService().getIP().then(async (res) => {
                if (deviceId.length == 0 || userAgent == undefined || userAgent.length == 0 || res.ip.length == 0) {
                    Promise.reject(new Response(undefined, { status: 400, statusText: "Bad Request" }))
                }
                return getAuthService().refresh(deviceId, res.ip, userAgent!).then((res) => {
                    if (res == 1) {
